...
//  Dtype* diff_vec = mutable_cpu_diff();
  if (proto.double_data_size() > 0) {
    ...
    if( is_quantization )
    {
        ...
        for ( int i = 0; i < count_; ++ i )
            data_copy[i] = std::fabs(data_copy[i] );
        std::sort( data_copy, data_copy + count_ );
      //Dtype max_data = data_copy[ count_ - 1 ];
	double EW=0.0;
	for (int i = (int)count_/1.5; i < count_; i ++)
		EW += data_copy[i];
	EW /= (count_ - (int)(count_/1.5));
	float partition=1.000; 
	PAIR a;
	for ( int i = 0; i < count_; ++ i){
		if( EW > 64)
		    break;
		srand(i+1); float randnum = (float)(rand()%1000)/1000.0;
		//if ( std::fabs( data_vec[i]) >= data_copy[partition])
		if( partition >= randnum){
			a = weightQuantify0( data_vec[i], 
				data_copy[(int)(count_/1.5)], EW );
			data_vec[i] = a.weight;
			mask_vec[i] = a.mask;
		}
	}
